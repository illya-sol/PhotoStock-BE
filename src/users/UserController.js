...
exports.find = (req, res) =>{
    db.FindById(req.params.user_id).then((result) => {res.status(302).send(result.fields);} )
    .catch((error) => { res.status(200).send({errors: '' + error}); });
}

exports.findbyname = (req, res) =>{
    db.FindbyUsername(req.params.username).then((result) => {
      if(result)
        res.sendStatus(302);
      else
        res.sendStatus(200);
...
exports.edit = (req, res) =>{
    ...
    db.Edit(req.body).then(() => {res.sendStatus(200);} )
